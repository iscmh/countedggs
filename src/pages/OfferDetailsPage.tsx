...
export default function OfferDetailsPage() {
  ...
  const getVideoId = (method: number, type: 'tutorial' | 'example', index: number = 0) => {
    if (offer.id === 'skore-ai') {
      return {
        tutorial: ['T1ubvsq7mXk', '2C52Q1QILYQ'],
        example: ['r8MOmQrs8GE', '7X-L9ZLGGCA']
      }[type][method - 1];
    } else {
      return {
        tutorial: ['veNSzAzBFPs', '2qRCpa8GI9Y'],
        example: ['rpu5Ega5uCs', 'dWvguEVTYvY', 'mLoOfbrqf6Y', 'r8NeBum0Xng'].slice(index * 3, (index + 1) * 3)
      }[type][method - 1];
    }
  };

  const getLogoUrl = () => {
    return offer.id === 'skore-ai' 
      ? 'https://iili.io/2u1pVcv.png'
      : 'https://iili.io/2u1yxON.png';
  };
  ...
  return (
    <div className="animate-fadeIn space-y-8">
      ...
      {(offer.id === 'skore-ai' || offer.id === 'faceless-twitter') && (
                  ...
                  <div className="grid grid-cols-3 gap-4">
                    {offer.id === 'faceless-twitter' ? (
                      Array(3).fill(null).map((_, index) => (
                        <div key={index} className="relative w-full" style={{ paddingBottom: '177.77%' }}>
                          <div className="absolute inset-0">
                            <YouTube
                              videoId={['rpu5Ega5uCs', 'mLoOfbrqf6Y', 'r8NeBum0Xng'][index]}
                              className="w-full h-full"
                              opts={{
                                width: '100%',
                                height: '100%',
